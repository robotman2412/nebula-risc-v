...
case class FetchFrag(cfg: NebulaCfg) extends Bundle {
    /** 16-bit instruction fragment. */
...
case class InsnExtractor(cfg: NebulaCfg, node: CtrlLink) {
    ...
    val logic = if (cfg.isa.C) new node.Area {
        ...
        val hw_insn     = Vec.fill(cfg.frontendWidth*2)(Bits(16 bits))
        /** Which halfwords are valid. */
        val hw_valid    = Bits(cfg.frontendWidth*2 bits)
        /** Which halfwords could be the start of a 32-bit instruction. */
        val hw_32bit    = Bits(cfg.frontendWidth*2 bits)
        /** PC value per halfword. */
        val hw_pc       = Vec.fill(cfg.frontendWidth*2)(SInt(cfg.vaddrWidth bits))
        
        // Infer per-halfword information.
        for (i <- 0 until cfg.frontendWidth*2) {
            val r = log2Up(cfg.frontendWidth*2)
        ...
        // Remaining halfwords.
        for (i <- 1 until cfg.frontendWidth*2) {
