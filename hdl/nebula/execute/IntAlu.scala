    ...
    val logic = new aluNode.Area {
      ...
      when(up(nebula.dispatch.Dispatch.SENDTOALU) === True && aluNode.isValid) {
        result := up(ALUOP).muxDc(
          ...
          AluOp.sll      -> (SRC1.asUInt |<< (SRC2(4 downto 0)).asUInt).asBits,
          AluOp.srl      -> (SRC1.asUInt |>> (SRC2(4 downto 0)).asUInt).asBits,
          AluOp.sra      -> (SRC1.asUInt >> (SRC2(4 downto 0)).asUInt).asBits,
          AluOp.sub      -> (SRC1.asSInt - SRC2.asSInt).asBits,

          AluOp.addw      -> (SRC1.asSInt + SRC2.asSInt)(31 downto 0).resize(64).asBits,
          AluOp.sllw      -> (SRC1.asSInt + SRC2.asSInt)(31 downto 0).resize(64).asBits,
          AluOp.sraw      -> (SRC1.asSInt + SRC2.asSInt)(31 downto 0).resize(64).asBits,
          AluOp.srlw      -> (SRC1.asSInt + SRC2.asSInt)(31 downto 0).resize(64).asBits,
          AluOp.subw      -> (SRC1.asSInt - SRC2.asSInt)(31 downto 0).resize(64).asBits,
        )
      }

      when(up(ALUOP) === AluOp.slt) {
        val slt = ((SRC1.asSInt - SRC2.asSInt).msb) ? True | False
        result := slt.asBits.resize(64)
        
      }
      when(up(ALUOP) === AluOp.sltu) {
        val sltu = ((SRC1.asUInt - SRC2.asUInt).msb) ? True | False
